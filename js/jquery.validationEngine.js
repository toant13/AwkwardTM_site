		...
		function _ajax(caller,rules,position){				 // VALIDATE AJAX RULES
			...
			if(!ajaxisError){
				$.ajax({
			  	 	...
			  	 	error: function(data,transport){ $.validationEngine.debug("error in the ajax1: "+data.status+" "+transport) },
	...
	submitForm : function(caller){
		if($.validationEngine.settings.ajaxSubmit){		
			...
			$.ajax({
				...
				url: "http://www.awkwardturtlemedia.com/ajaxSubmit.php",
			   	async: true,
			   	data: $(caller).serialize()+"&"+extraData,
			   	//error: function(data,transport){ $.validationEngine.debug("error in the ajax2: "+data.status+" "+transport) },
				
				error: function(data,transport)  //temporary error hack
				{
			   		if(data.status ==0)
					{
					...
					{
						$.validationEngine.debug("error in the ajax2: "+data.status+" "+transport);
					}
